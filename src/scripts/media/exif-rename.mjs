...
for (const file of allFiles) {
  if (!file.toLowerCase().endsWith('.jpeg')) continue;
  const newFile = file.replace(/\.jpeg$/i, '.jpg');
  runOrThrow('convert', [file, newFile]);
  fs.unlinkSync(file);
}
...
const imageFiles = walkFiles(dir).filter((file) => {
  ...
  return /\.(jpg|heic|png)$/i.test(file);
});
...
const renamedFiles = walkFiles(dir).filter((file) => /\.(jpg|heic|png)$/i.test(file));
